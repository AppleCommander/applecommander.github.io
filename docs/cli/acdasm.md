`acdasm` is the AppleCommander disassembler. It is embedded within the AppleCommander application as well as the `acx` tool. However, the dedicated tool has more control over its capabilities. The downloads can be found at the [acdasm](https://github.com/AppleCommander/acdasm) project.
